    ...
    <div className="rcard">
      <div className="rcard-img">
        ...
        <div className="rcard-badges">
          ...
          {item.operator && <Badge tone="info">{t('rental.op.yes')}</Badge>}
        </div>
      </div>
      <div className="rcard-body">
        <div className="rcard-brand">{item.brand} · {item.year} · {item.hours.toLocaleString('ru-RU')} {t('rcard.hours')}</div>
        ...
        <div className="rcard-rates">
          <div><span>{t('rcard.shift')}</span><b>{fmtKZT(item.rates.shift)}</b></div>
          <div><span>{t('rcard.day')}</span><b>{fmtKZT(item.rates.day)}</b></div>
          <div><span>{t('rcard.week')}</span><b>{fmtKZT(item.rates.week)}</b></div>
          <div><span>{t('rcard.month')}</span><b>{fmtKZT(item.rates.month)}</b></div>
        </div>
