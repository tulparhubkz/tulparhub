...
function deliveryDaysKey(city: string): TranslationKey {
  const map: Record<string, TranslationKey> = {
    'Алматы': 'pdp.eta.today', 'Астана': 'pdp.eta.d12',
    'Шымкент': 'pdp.eta.d23', 'Уральск': 'pdp.eta.d34',
    'Бишкек': 'pdp.eta.d23', 'Павлодар': 'pdp.eta.d23',
  }
  return map[city] ?? 'pdp.eta.d35'
}
...
function tierLabelKey(seg: string): TranslationKey {
  if (seg === 'Премиум') return 'pdp.tier.prem'
  if (seg === 'Средний') return 'pdp.tier.mid'
  return 'pdp.tier.budget'
}
...
export default function PDPPage() {
  ...
  if (!part || part.error) return (
    <div className="container" style={{ padding: '6rem 0', textAlign: 'center' }}>
      <h2>{t('pdp.notFound')}</h2>
      <Link href="/catalog" style={{ color: 'var(--accent)', marginTop: 16, display: 'inline-block' }}>{t('pdp.backToCatalog')}</Link>
    </div>
  )
    ...
    <>
      ...
      <div className="container">
          ...
          <div className="pdp-info">
            ...
            <div className="pdp-badges">
              <span className={`badge ${isOEM ? 'oem' : ''}`} style={!isOEM ? { background: 'var(--surf-2)', color: 'var(--ink-2)' } : {}}>
                {isOEM ? t('pdp.badge.oem') : t('pdp.badge.analog')}
              </span>
              {totalQty > 0 ? (
                <span className="badge stock">
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                  {t('pdp.badge.inStockPre')}{stockEntries.length} {t('pdp.badge.whShort')}
                </span>
              ) : (
                <span className="badge" style={{ background: 'var(--surf-2)', color: 'var(--ink-2)' }}>{t('pdp.badge.onOrder')}</span>
              )}
            </div>
            ...
            <div className="buybox">
              ...
              <div className="buybox-actions">
                ...
                >
                  ...
                  {inWish ? t('pdp.inWish') : t('pdp.toWish')}
                </button>
              </div>
                ...
                <div className="stock-list">
                  ...
                  {stockEntries.map(([city, q]) => (
                    <div key={city} className={`stock-row${city === warehouseCity(myCity) ? ' mine' : ''}`}>
                      ...
                      <span className="eta">{t(deliveryDaysKey(city))}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="pdp-sections">
                ...
                {(part.fits as string[]).map((f: string) => (
                  <Link key={f} href={`/catalog?q=${encodeURIComponent(f)}`} className="fit">
                    ...
                    <div>
                      ...
                      <div className="yr" style={{ fontSize: 13, color: 'var(--ink-3)' }}>{t('pdp.viewParts')}</div>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {/* Аналоги */}
          <div className="psec">
            <h2>{t('pdp.sec.analogs')}</h2>
            <div className="analogs">
              {/* Current item */}
              <div className="acard current">
                <span className="curmark">● {t('pdp.current')}</span>
              ...
              {analogs.map((a) => {
                ...
                return (
                  ...
                  >
                    <span className={`atier ${tierClass(aSeg)}`}>{t(tierLabelKey(aSeg))}</span>
              ...
              {analogs.length === 0 && (
                <div className="acard" style={{ opacity: 0.4, pointerEvents: 'none' }}>
                  <span className="atier budget">{t('pdp.searching')}</span>
                  <div className="aname">{t('pdp.searchingText')}</div>
