    ...
    <>
      ...
      <main className="container pb-page">
        <div className="crumbs" style={{ marginBottom: 20 }}>
          <Link href="/">{t('common.home')}</Link> / <span>{t('nav.brands')}</span>
        </div>
        <h1 className="pb-title">{t('pbrands.title')}</h1>
        <p className="pb-sub">{brands.length} {t('pbrands.subWord')}</p>

        <input
          className="pb-search"
          placeholder={t('pbrands.searchPh')}
          value={q}
            ...
            {filtered.map(b => {
              ...
              return (
                <Link key={b.name} href={`/parts-brands/${encodeURIComponent(b.name)}`} className="pb-card">
                  ...
                  <div className="pb-count">{b.count} {b.count === 1 ? t('pbrands.posOne') : t('pbrands.posMany')}</div>
