...
// same-origin targets — never become an open redirect.
...
function AuthInner() {
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    setError('')
    setLoading(true)
    const res = await signIn('nodemailer', { email, redirect: false, callbackUrl: safeCallback(params.get('callbackUrl')) })
    setLoading(false)
    if (res?.error) {
        ...
        <div style={{ padding: 32 }}>
          {sent ? (
            ...
            <>
              ...
              {/* Google */}
              <button type="button" onClick={() => signIn('google', { callbackUrl: safeCallback(params.get('callbackUrl')) })}
