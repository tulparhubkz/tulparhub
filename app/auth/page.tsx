...
export default function AuthPage() {
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    setLoading(false)
    if (res?.error) {
      setError('Вход по email пока не настроен. Попробуйте Google.')
    } else {
      setSent(true)
    }
  }
        ...
        <div style={{ padding: 32 }}>
          {sent ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 12 }}>Проверьте почту</h2>
              <p style={{ color: 'var(--ink-2)', fontSize: 14, lineHeight: 1.6 }}>
                Мы отправили ссылку для входа на <b>{email}</b>. Перейдите по ней, чтобы войти.
              </p>
            </div>
          ) : (
            <>
              <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Войдите или создайте аккаунт</h2>
              <p style={{ color: 'var(--ink-2)', fontSize: 14, marginBottom: 24 }}>Чтобы видеть заказы, избранное и историю</p>
